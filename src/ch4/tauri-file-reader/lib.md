# lib.rsをJavaScript風に解説したもの

これは、[src-tauri/src/lib.rs](src-tauri/src/lib.rs) を解説したものです。**「Rustで書いた Tauri のバックエンド処理を、JavaScriptの感覚でどう見るか」** という話にしています。

JavaScript風に解説していますが、**Tauri のこの部分は本来 Rust でしか書けません**。なのでここでは、**同じ役割を JavaScript で表すとこんな感じ**、という対応で説明します。
